		  ...
		  <div class="row-fluid">
			...
			<?php if ( have_posts() ) : ?>
	    	  ...
	    	  <div class="category-title" style="margin-bottom: 20px;">
			     <h1 class="entry-title" style="margin-bottom:0;"><?php printf( __( 'Author Archives: %s', 'color-theme-framework' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' ); ?></h1>
		      </div> <!-- /pages-title -->
			  ...
			  <div id="entry-blog">
			    <?php while ( have_posts() ) : the_post(); ?>
				  <article id="post-<?php the_ID(); ?>" <?php post_class('entry-post'); ?>>	
					...
					<?php if ($data['crossroad_blog_show_meta']['author'] == true ) : ?><span class="meta-author">posted by <?php echo the_author_link(); ?></span><?php endif; ?>
								...
								if( $videoid != '' ) : ?>
									...
									<?php
										...
										elseif ( $video_type == 'dailymotion' ) {
		            						if ( $thumb_type == 'auto' ) {
			            						echo '<img src="' . getDailyMotionThum($videoid) . '" alt="'. the_title('','',false) . '" />';
											} 
											else if ( $thumb_type == 'featured' && has_post_thumbnail() ) {
												$small_image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), 'blog-thumb');
												echo '<img src="' . $small_image_url[0] . '" alt="'. the_title('','',false) . '" />';
