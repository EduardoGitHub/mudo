...
// the image filename as default
  ...
  // The HTML image wrapper function
  function tep_image_galeria($src, $alt = '', $redimencionar = 'NO', $tamanhoRedm = 'P', $parameters = '') {
// alt is added to the img tag even if it is null to prevent browsers from outputting
// the image filename as default
	$image_size = @getimagesize($src);
	if($redimencionar == 'NO'){
		...
		if(($image_size[2] <> 1) && ($image_size[2] <> 2) && ($image_size[2] <> 3) ){
			$new_name = reduzir($src,$tamanhoRedm);
			$image = '<img src="'.HTTP_SERVER_IMAGES. tep_output_string($new_name[0]) . '" border="0" alt="' . tep_output_string($alt) . '"';
			if (tep_not_null($alt)) {
			  $image .= ' title=" ' . tep_output_string($alt) . ' "';
			}
			if (tep_not_null($parameters)) $image .= ' ' . $parameters;
...
// Output a form pull down menu
  ...
  function tep_draw_pull_down_menu_SEO($name, $values, $default = '', $parameters = '', $required = false) {
    ...
    for ($i=0, $n=sizeof($values); $i<$n; $i++) {
	  ...
	  if(isset($values[$i]['text_name'])) $desc = $values[$i]['text_name']; else $desc = $values[$i]['text'];
      $field .= '>' . tep_output_string($desc, array('"' => '&quot;', '\'' => '&#039;', '<' => '&lt;', '>' => '&gt;')) . '</option>';
    }
    $field .= '</select>';
