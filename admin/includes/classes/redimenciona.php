...
function resizeImage($im, $la='50%', $al = '0', $type)
{
    ...
    echo "falha ao copiar $name_image...\n";
	}
			...
			$caminho = '../'.DIR_WS_IMAGES_PRODUTOS.$name_already_resize;
				if(file_exists($caminho)){
					...
					return $info_images;
						...
						if(($width < $largura_definida)and($height < $altura_definida)){
						if($width > $height){
								$image_width = $width ;
								$image_height = 0;
						...
						}else if(($width > $largura_definida)and($height < $altura_definida)){
...
function clearimg(){
	$product_query = tep_db_query("SELECT products_image as Imagem FROM PRODUCTS union SELECT products_extra_image FROM PRODUCTS_extra_images");
    $product = tep_db_fetch_array($product_query);
		while($product = tep_db_fetch_array($product_query)) {
			$new_name = substr($product['Imagem'],0,-4);
			$type_file = substr($product['Imagem'],-4);
			$name_already_resize = "R".$new_name."_".SMALL_IMAGE_WIDTH."x".SMALL_IMAGE_HEIGHT.$type_file;
			$caminho = '../'.DIR_WS_IMAGES_PRODUTOS.$name_already_resize;
			$caminho2 = '../'.DIR_WS_IMAGES_PRODUTOS."R".$new_name."_80x80".$type_file;
			...
			chmod($caminho2, 0777);   
			$cleaning = unlink($caminho2);
			}

		}
	return "Todas as imagens redimencionadas foram deletadas com sucesso.";	
}
