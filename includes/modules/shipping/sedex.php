...
{ 
  ...
  { 
    ...
    $this->icon = '';
    $this->enabled = ((MODULE_SHIPPING_SEDEX_STATUS == 'True') ? true : false);
    $this->sedex = 1;
  } 
...
// class methods 
  ...
  { 
    ...
    */
	...
	if (MODULE_SHIPPING_SEDEX_VALORD=='Sim'){
    ...
    $this->quotes = array('id' => $this->code, 'module' =>MODULE_SHIPPING_SEDEX_TEXT_TITLE, 'methods' => array(array('id' => $this->code, 'title' => 'O prazo estimado de produção + entrega para sua região é de '.($frete[1]+5).' dias utéis via SEDEX', 'cost'
=>$shipping_cost = ($shipping + MODULE_SHIPPING_SEDEX_HANDLING + SHIPPING_HANDLING)))); 
    if ($shipping > 0) 
      return $this->quotes;
    else 
      return $this->quotes['error'] = MODULE_SHIPPING_SEDEX_INVALID_ZONE; 
  ...
  { 
    ...
    { 
      $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_SEDEX_STATUS'");$this->_check =tep_db_num_rows($check_query);
    } 
    return $this->_check; 
  } 
  ...
  function install() 
  { 
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Entrega via Sedex', 'MODULE_SHIPPING_SEDEX_STATUS', 'True', 'Ativar Entrega via Sedex (cálculo online via Correios)?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())"); 

	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Taxa de manipulação', 'MODULE_SHIPPING_SEDEX_HANDLING', '0', 'Taxa de manipulação para esta forma de envio.', '6', '0', now())");

    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Seguro sobre o Valor Declarado', 'MODULE_SHIPPING_SEDEX_VALORD', 'Não', 'Adicionar seguro sobre o valor declarado da encomenda (para valores acima de R$ 100,00)? - Consulte os correios. Esta opção aumenta o valor do frete, mas a mercadoria fica assegurada pelos Correios.', '6', '0', 'tep_cfg_select_option(array(\'Sim\',\'Não\'), ', now())");
    
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Mensagem na tela de frete', 'MODULE_SHIPPING_SEDEX_MSG', 'Prazo de Entrega: 1 a 3 dias úteis após a postagem.', 'Mensagem que será exibida de tela de seleção da forma de envio.', '6', '0', now())");
	
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Serviço Mão Propria', 'MODULE_SHIPPING_SEDEX_MAOPROPRIA', 'N', 'Indica se a encomenda será entregue com o serviço adicional mão própria.<br/>S ou N (S – Sim, N – Não) ', '6', '0', now())");
		
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Serviço Aviso de Recebimento', 'MODULE_SHIPPING_SEDEX_AVRECEBIMENTO', 'N', 'Indica se a encomenda será entregue com o serviço adicional aviso de recebimento.<br />S ou N (S – Sim, N – Não)', '6', '0', now())");
    
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Ordem de exibição.', 'MODULE_SHIPPING_SEDEX_SORT_ORDER', '2', 'Determina a ordem de exibição desta forma de envio.', '6', '0', now())"); 
...
$resposta = 'xml';
 ...
 $conexao = fsockopen($webservice, 80, $errno, $errstr, 30);
    ...
    { 
      //$saida = "GET /calculador/CalcPrecoPrazo.aspx?ncdempresa=&sdssenha=&sceporigem=$cepOrigem&scepdestino=$cepDestino&nvlpeso=$peso&ncdformato=1&nvlcomprimento=30&nvlaltura=25&nvllargura=30&scdmaopropria=n&nvlvalordeclarado=$valor&scdavisorecebimento=n&ncdservico=$servico&nvldiametro=0&strretorno=$resposta HTTP/1.1\r\n";
...
return array(number_format( $preco_postal, 2, ".", "," ),$prazo[0]);
  ...
  function keys() 
  { 
     $keys = array('MODULE_SHIPPING_SEDEX_STATUS', 'MODULE_SHIPPING_SEDEX_HANDLING', 'MODULE_SHIPPING_SEDEX_VALORD', 'MODULE_SHIPPING_SEDEX_MSG','MODULE_SHIPPING_SEDEX_MAOPROPRIA','MODULE_SHIPPING_SEDEX_AVRECEBIMENTO','MODULE_SHIPPING_SEDEX_SORT_ORDER'); 
