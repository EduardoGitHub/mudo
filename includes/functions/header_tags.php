...
function tep_header_tag_page($file) {
  ...
  if ($pageTags['append_default_title'] && isset($tmpTags['def_title'])) $sortOrder['title'][$pageTags['sortorder_title']] = $tmpTags['def_title'];
  if ($pageTags['append_default_description'] && isset($tmpTags['def_desc'])) $sortOrder['description'][$pageTags['sortorder_description']] = $tmpTags['def_desc'];
  if ($pageTags['append_default_keywords'] && isset($tmpTags['def_keywords'])) $sortOrder['keywords'][$pageTags['sortorder_keywords']] = $tmpTags['def_keywords'];
  if ($pageTags['append_default_logo'] && isset($tmpTags['def_logo_text']))  $sortOrder['logo'][$pageTags['sortorder_logo']] = $tmpTags['def_logo_text'];
  ...
  if (! isset($header_tags_array['title'])) $header_tags_array['title'] = $pageName . (isset($tmpTags['def_title']) ? HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $tmpTags['def_title'] : '');
  if (! isset($header_tags_array['description'])) $header_tags_array['description'] = $pageName . (isset($tmpTags['def_desc']) ? HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $tmpTags['def_desc'] : '');
  if (! isset($header_tags_array['keywords'])) $header_tags_array['keywords'] = $pageName . (isset($tmpTags['def_keywords']) ? HEADER_TAGS_SEPARATOR_KEYWORD . ' ' . $tmpTags['def_keywords'] : '');
  if (! isset($header_tags_array['logo']))  $header_tags_array['logo'] = $pageName . (isset($tmpTags['def_logo_text']) ? HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $tmpTags['def_logo_text'] : '');
...
function GetCategoryAndManufacturer($sortOrder, $pageTags, $defaultTags, $catStr, $manStr, $product = false)
{
  ...
  if (($type == 'cat' || $type == 'top') && ($pageTags['append_category'] || $defaultTags['default_logo_append_group'] || $defaultTags['default_logo_append_category']))
  {
    if ($category_depth == 'nested' || $category_depth == 'products' || $product)
    {
      ...
      if (isset($the_category['htc_title_tag']))
      {
        $catTitle = isset($parentStr) ? ($parentStr . $the_category['htc_title_tag']) : $the_category['htc_title_tag'];  
        $sortOrder['title'][$pageTags['sortorder_category']] = isset($sortOrder['title'][$pageTags['sortorder_category']]) ? $sortOrder['title'][$pageTags['sortorder_category']] . ' ' . HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $catTitle : $catTitle;
        $sortOrder['logo'][$pageTags['sortorder_category']] = isset($sortOrder['logo'][$pageTags['sortorder_category']]) ? $sortOrder['logo'][$pageTags['sortorder_category']] . ' ' . HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $catTitle : $catTitle;
      }
      if (isset($the_category['htc_desc_tag']))
      {
        $catDesc = isset($parentStr) ? ($parentStr . $the_category['htc_desc_tag']) : $the_category['htc_desc_tag'];  
        $sortOrder['description'][$pageTags['sortorder_category']] = isset($sortOrder['description'][$pageTags['sortorder_category']]) ? $sortOrder['description'][$pageTags['sortorder_category']] . ' ' . HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $catDesc : $catDesc;
      }
      if (isset($the_category['htc_keywords_tag']))
      {
		  	$catkeywords ='';
        $catKeywords = isset($parentStr) ? (str_replace(HEADER_TAGS_SEPARATOR_DESCRIPTION, HEADER_TAGS_SEPARATOR_KEYWORD, $parentStr) . $the_category['htc_keywords_tag']) : $the_category['htc_keywords_tag'];  
        $sortOrder['keywords'][$pageTags['sortorder_category']] = isset($sortOrder['keywords'][$pageTags['sortorder_category']]) ? $sortOrder['keywords'][$pageTags['sortorder_category']] . ' ' . HEADER_TAGS_SEPARATOR_KEYWORD . ' ' . $catkeywords : $catKeywords;
      }
    }
  }
  ...
  if ($type == 'man' && ($pageTags['append_manufacturer'] || $defaultTags['default_logo_append_group'] || $defaultTags['default_logo_append_manufacturer']))
  {
    ...
    if (isset($the_manufacturer['htc_title_tag']))
    {
      $sortOrder['title'][$pageTags['sortorder_manufacturer']] = isset($sortOrder['title'][$pageTags['sortorder_manufacturer']]) ? $sortOrder['title'][$pageTags['sortorder_manufacturer']] . ' ' . HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $the_manufacturer['htc_title_tag'] : $the_manufacturer['htc_title_tag'];
      $sortOrder['logo'][$pageTags['sortorder_manufacturer']] = isset($sortOrder['logo'][$pageTags['sortorder_manufacturer']]) ? $sortOrder['title'][$pageTags['sortorder_manufacturer']] . ' ' . HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $the_manufacturer['htc_title_tag'] : $the_manufacturer['htc_title_tag'];
    }
    if (isset($the_manufacturer['htc_desc_tag']))
    {
      $sortOrder['description'][$pageTags['sortorder_manufacturer']] = isset($sortOrder['description'][$pageTags['sortorder_manufacturer']]) ? $sortOrder['title'][$pageTags['sortorder_manufacturer']] . ' ' . HEADER_TAGS_SEPARATOR_DESCRIPTION . ' ' . $the_manufacturer['htc_desc_tag'] : $the_manufacturer['htc_desc_tag'];
      $sortOrder['description'][$pageTags['sortorder_manufacturer']] = $the_manufacturer['htc_desc_tag'];
    }
    if (isset($the_manufacturer['htc_keywords_tag']))
    {
      $sortOrder['keywords'][$pageTags['sortorder_manufacturer']] = isset($sortOrder['keywords'][$pageTags['sortorder_manufacturer']]) ? $sortOrder['title'][$pageTags['sortorder_manufacturer']] . ' ' . HEADER_TAGS_SEPARATOR_KEYWORD . ' ' . $the_manufacturer['htc_keywords_tag'] : $the_manufacturer['htc_keywords_tag'];
    }
  }
  return $sortOrder;
}
