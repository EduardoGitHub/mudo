...
// Display a banner from the specified group or banner id ($identifier)
  function tep_display_banner($action, $identifier) {
    ...
    if (tep_not_null($banner['banners_html_text'])) {
	  ...
	  $size = getimagesize(DIR_WS_IMAGES_BANNERS . $banner['banners_image']);
      $banner_string = mm_output_flash_movie( $banner['banners_title'], DIR_WS_IMAGES_BANNERS . $banner['banners_image'] , $size[0]  , $size[1]);
	  } else {
		$pos = strrpos($banner['banners_url'], "http");
		if ($pos === false) { // note: três sinais iguais
			$linkext = '';
			$ex = '&ext=no';
		}else{
				$linkext = 'target="_blank"';
				$ex = '&ext=yes';
		}
	  $banner_string = '<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner'.$ex.'&goto=' . $banner['banners_id']) . '" '.$linkext.'>' . tep_image(DIR_WS_IMAGES_BANNERS . $banner['banners_image'], $banner['banners_title']) . '</a>';
	  }
	}
...
// Check to see if a banner exists
  function tep_banner_exists($action, $identifier) {
    if ($action == 'dynamic') {
      ...
      $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_url, banners_html_text from " . TABLE_BANNERS . " where status = '1' and banners_id = '" . (int)$identifier . "'");
