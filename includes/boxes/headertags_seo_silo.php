...
//  $silo_query = tep_db_query("select * case category_id = " . (int)$catID . " from " . TABLE_HEADERTAGS_SILO . " where ( category_id = 0 or category_id = " . (int)$catID . " ) and language_id = " . (int)$languages_id);
  ...
  if (tep_db_num_rows($products_query) > 0)
  {
    ...
    {
      $linkList .= '&raquo; <a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products['products_id']) . '" title="' . $products['products_head_title_tag'] . '">' . $products['products_head_title_tag'] . '</a><br />';
    } 
  }
  else
  {
    ...
    if ($subCnt > 0)
    {
      ...
      while($subcategories = tep_db_fetch_array($subcategories_query))
        $linkList .= '&raquo; <a href="' . tep_href_link(FILENAME_DEFAULT, 'cPath=' . $subcategories['categories_id']) . '" title="' . $subcategories['categories_htc_title_tag'] . '">' . $subcategories['categories_htc_title_tag'] . '</a><br />';
    }
  }
  ...
  if (empty($linkList))
    return;
?>
<div class="box" id="vejatambem">
<div class="lay_bordaBox"><span><?=$silo['box_heading']?></span></div>
<div class="boxconteudo" style="font-family:Verdana, Geneva, sans-serif; font-size:11px; text-transform:lowercase;">
<?=$linkList?>
</div>
</div>
