<?php
/*
  $Id: whos_online.php,v 3.3 2007/11/08 nerbonne Exp $
...
// Images used for status lights
  $status_active_cart = 'icon_status_cart.gif';
...
// Pull Session data from the correct source.
    ...
    if ((STORE_SESSIONS == 'mysql') or (STORE_SESSIONS == '')) {  // Modified by Erick Cedano
...
/* END COUNT MOD */
 ...
 while($ec < count($bot_name)) {
	 if (strstr(gethost($whos_online['ip_address']),$bot_name[$ec])) {
	    $ecbot = true;
	 }
	$ec++;
	}
...
?>
 ...
 //                   echo '<a HREF="customers.php?selected_box=customers&cID=' . $whos_online['customer_id'] . '&action=edit">' . $whos_online['full_name'] . '</a>';
		...
		?>
                    <a HREF="<?php echo ('http://www.showmyip.com/?ip=' . $whos_online['ip_address'] . '&get=nmap');?>" target="_blank">
<font color="<?php echo $fg_color; ?>">
    ...
    echo gethost($whos_online['ip_address']);
		...
		$temparr=split("\/",$temp);
		$products_id=$temparr[2];

		if ($products_id) {
			if ($products_id=='products_id') { $products_id=$temparr[3]; }
			else { $products_id=$temparr[5]; }
			}
		else	{
...
if ($products_id == '') $products_id =  substr($temp_url_link,strpos($temp_url_link,"products_id=")+12);
...
-->
<?php print "<b>" . TEXT_MY_IP_ADDRESS . ":</b>&nbsp;".$_SERVER["REMOTE_ADDR"]; ?><br><?php echo TEXT_NOT_AVAILABLE;?></td>
<br>

<?php
//   $box = new box;
//   $contents = sizeof($products);
...
// Added by Graphicore
   echo '<div class="smalltext">' . ($products[$i]['quantity'] . ' x ' . $products[$i]['name'] . '</div>');
// EOA
