...
//funcao para conveter semana do ingles para portugues
	...
	function mesReferenteEN($diaSemana){ 
		switch ($diaSemana) {
			case 'Janeiro':   return 'January';   break;
			case 'Fevereiro': return 'February'; break;
			case 'Março':     return 'March';     break;
			case 'Abril':     return 'April';     break;
			case 'Maio':      return 'May';      break;
			case 'Junho':     return 'June';     break;
			case 'Julho':     return 'July';     break;
			case 'Agosto':    return 'August';    break;
			case 'Setembro':  return 'September';  break;
			case 'Outubro':   return 'October';   break;
			case 'Novembro':  return 'November';  break;
			case 'Dezembro':  return 'December';  break;
		}
	}
...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function tep_info_image($image, $alt, $width = '', $height = '', $params ='') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES . $image)) ) {
	  //REDIMENSIONAMENTO DE IMAGEM
			...
			$type_ofimage = @getimagesize("../".DIR_WS_IMAGES_PRODUTOS.$image);	
			
			$vl_w ='';
			$vl_h = '';	
			if(($type_ofimage[2] <> 1) && ($type_ofimage[2] <> 2) && ($type_ofimage[2] <> 3) ){
				$new_name = $image;
				$dimensao = reduzir($image);
				$vl_w = $dimensao[0];
				$vl_h = $dimensao[1];
			}else{
					$new_name = redimenciona($image,'SP');
				 }
		//FIM REDIMENCIONAMENTO DE IMAGEM
		
		$image = tep_image($new_name[0], $alt, $new_name[1], $new_name[2],$params);
  ...
  function tep_info_image2($image, $alt, $width = '', $height = '') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES.'Fotos/' . $image)) ) {
      $image = tep_image(DIR_WS_CATALOG_IMAGES.'Fotos/' . $image, $alt, $width, $height);
    } else {
      $image = TEXT_IMAGE_NONEXISTENT;
    }
  ...
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
	...
	$number = tep_output_string_protected($address['number']);
	//$street2 = tep_output_string_protected($address['street_address2']);
	//$street3 = tep_output_string_protected($address['street_address3']);
    ...
    $zip = tep_putcaracters($postcode, '#####-###');
	if($complemento <> ''){
		$texcomplemnto = ' / '.$complemento;
	}else{
		$texcomplemnto = '';
	}
...
$statecomma = ''; 
$streets = $street; 
if ($street2 != '') $streets = $street;
if ($suburb != '') $streets = $street. ', ' . $number. $texcomplemnto .$cr . $suburb; 
...
if ($state != '') $statecomma =$state . ', ';
  ...
  function tep_get_products_description_simple($product_id, $language_id) {
    $product_query = tep_db_query("select products_description_simple from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_description_simple'];
  }
  ...
  function tep_get_products_availability($product_id, $language_id) {
    $product_query = tep_db_query("select products_availability from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);
...
// Sets the status of a partner
  function tep_set_partners_status($banners_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_PARTNERS . " set status = '1', expires_impressions = NULL, expires_date = NULL, date_status_change = NULL where banners_id = '" . $banners_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_PARTNERS . " set status = '0', date_status_change = now() where banners_id = '" . $banners_id . "'");
    } else {
      return -1;
    }
  }  
...
// Sets the status of a riviews
  function tep_set_reviews_status($reviews_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_REVIEWS . " set status = '1', last_modified = now() where reviews_id = '" . (int)$reviews_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_REVIEWS . " set status = '0', last_modified = now() where reviews_id = '" . (int)$reviews_id . "'");
    } else {
      return -1;
    }
  }  
...
// Sets the status of a product
  function tep_set_categories_highlights($categories_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_CATEGORIES . " set categories_highlights = '1', last_modified = now() where categories_id = '" . (int)$categories_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_CATEGORIES . " set categories_highlights = '0', last_modified = now() where categories_id = '" . (int)$categories_id . "'");
    } else {
      return -1;
    }
  }  
...
// Retreive server information
  ...
  function tep_remove_product($product_id) {
	...
	//DELETA IMAGENS EXTRA
	$image_extra_query = tep_db_query("select products_extra_image, products_extra_images_id from products_extra_images where products_id = '" . $product_id . "'");
...
// Output the tax percentage with optional padded decimals
  ...
  function tep_sendMail($to_email_address, $email_subject, $html, $from_email_name, $from_email_address,$anexo=''){ //NOVO
  
  	$smtp_endereco = ENTRY_EMAIL_HOST; // coloque aqui seu endereço ou ip do servidor SMTP
	$usuario_smtp = ENTRY_EMAIL_USER; // coloque aqui seu usuário SMTP em geral é o próprio email que envia.
	$senha_smtp = ENTRY_EMAIL_PASSWORD; // coloque aqui sua senha para o usuário SMTP acima.
	
	//$mensagem_sucesso = $_POST['SuccessDocument']; // coloque aqui sua mensagem de sucesso ao envio do formulário.
	$seu_email = $to_email_address; //'cadastro@brimempresas.com.br'; // coloque aqui o endereço de email que deseja receber as mensagems do formulário de contato.
	...
	$mail->SMTPSecure = ENTRY_EMAIL_COMUNICATION; // ajusto o tipo de comunicação a ser utilizada, no caso, a TLS do GMail
	...
	<html>
		...
		<table width="760" border="0" cellspacing="0" cellpadding="0" style="border:1px solid #CCC; padding:15px; background-color:#CC6E6E" align="center">
          ...
          <tr>
			<td>
            	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                    <td style="color:#FFF; font-weight:bold; text-align:right; padding-right:3px; font-family:Calibri;">wwww.mudominhacasa.com.br</td>
                  </tr>
                </table>
            </td>
		  </tr>
		  <tr>
			<td class="texto">
            	<table width="760" border="0" cellspacing="0" cellpadding="0" style="background-color:#FFF; margin:5px; padding:5px; font-family:Calibri;">
					<tr><td>'.$html.'</td></tr>
				<table>
			</td>
		  </tr>	
	...
	{ //Checa erros no envo do email
		echo "Erro: " . $mail->ErrorInfo; //Imprime mensagem de que email nào foi enviado
		exit; 
	}
...
// TABLES: tax_rates
  ...
  function tep_get_type_sedex($sedex_class_id) {
    if ($sedex_class_id == '0') {
      return TEXT_NONE;
    } else {
      $classes_query = tep_db_query("select name_ofsedex from " . TABLE_SEDEX . " where value_ofsedex = '" . (int)$sedex_class_id . "'");
      $classes = tep_db_fetch_array($classes_query);

      return $classes['name_ofsedex'];
    }
  }
  
  function tep_cfg_pull_down_type_sedex($sedex_class_id, $key = ''){
  
	$name = (($key) ? 'configuration[' . $key . ']' : 'configuration_value');
    $zone_class_array = array(array('id' => '0', 'text' => TEXT_NONE));
    $zone_class_query = tep_db_query("select name_ofsedex, value_ofsedex from " . TABLE_SEDEX . " order by id_sedex");
    while ($zone_class = tep_db_fetch_array($zone_class_query)) {
      $zone_class_array[] = array('id' => $zone_class['value_ofsedex'],
                                  'text' => $zone_class['name_ofsedex']);
    }
...
/*** End Header Tags SEO ***/
  ...
  if (!extension_loaded('gd') && !extension_loaded('gd2')) {
				trigger_error("Você não possui a biblioteca GD.", E_USER_WARNING);
				return false;
		}
	  ...
	  switch ($imgInfo[2]) {
		...
		default:  trigger_error('Tipo de imagem não reconhecido. Apenas imagens PNG, JPEG, JPG e GIF são aceitas', E_USER_WARNING);  break;
	  }
