...
// class methods 
  ...
  { 
    ...
    */
    ...
    $this->quotes = array('id' => $this->code, 'module' =>MODULE_SHIPPING_PAC_TEXT_TITLE, 'methods' => array(array('id' => $this->code, 'title' => 'O prazo estimado de produção +  entrega para sua região é de '.($frete[1]+5).' dias utéis via PAC', 'cost'
=>$shipping_cost = ($shipping + MODULE_SHIPPING_PAC_HANDLING + SHIPPING_HANDLING)))); 
  ...
  function install() 
  { 
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Entrega via PAC', 'MODULE_SHIPPING_PAC_STATUS', 'True', 'Ativar Entrega via PAC (cálculo online via Correios)?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())"); 
														   
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Taxa de manipulação', 'MODULE_SHIPPING_PAC_HANDLING', '0', 'Taxa de manipulação para este módulo de transporte.', '6', '0', now())");

    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Seguro sobre o Valor Declarado', 'MODULE_SHIPPING_PAC_VALORD', 'Não', 'Adicionar seguro sobre o valor declarado da encomenda (para valores acima de R$ 100,00)? - Consulte os correios. Esta opção aumenta o valor do frete, mas a mercadoria fica assegurada pelos Correios.', '6', '0', 'tep_cfg_select_option(array(\'Sim\',\'Não\'), ', now())");
    
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Mensagem na tela de frete', 'MODULE_SHIPPING_PAC_MSG', 'Prazo de Entrega: 3 a 12 dias úteis após a postagem.', 'Mensagem que será exibida de tela de seleção da forma de envio.', '6', '0', now())");
	

		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Serviço Mão Propria', 'MODULE_SHIPPING_PAC_MAOPROPRIA', 'N', 'Indica se a encomenda será entregue com o serviço adicional mão própria.<br/>S ou N (S – Sim, N – Não) ', '6', '0', now())");
    ...
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Ordem de exibição.', 'MODULE_SHIPPING_PAC_SORT_ORDER', '1', 'Determina a ordem de exibição desta forma de envio.', '6', '0', now())"); 
