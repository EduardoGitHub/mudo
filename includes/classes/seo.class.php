<?php
define('USE_SEO_REDIRECT_DEBUG', 'true');
/**
 ...
 */        
        function SEO_URL_INSTALLER(){
                ...
                $this->default_config['SEO_URLS_CACHE_RESET'] = array('DEFAULT' => 'false',
                                      'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES (NULL, 'Limpar SEO URLs Cache', 'SEO_URLS_CACHE_RESET', 'false', 'Limpa o cache das URL amigaveis', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), 'tep_reset_cache_data_seo_urls', 'tep_cfg_select_option(array(''reset'', ''false''),')");
                $x++;
                $this->default_config['SEO_URLS_UNINSTALL'] = array('DEFAULT' => 'false',
                                      'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES (NULL, 'Remover o modelo de URL SEO', 'SEO_URLS_DB_UNINSTALL', 'false', 'Esta aзгo removerб todos os dados referente a URL amigavel', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), 'tep_reset_cache_data_seo_urls', 'tep_cfg_select_option(array(''uninstall'', ''false''),')");
                $this->init();
 ...
 */
        function href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = true){
                ...
                if ( !in_array($page, $this->attributes['SEO_PAGES']) || $this->attributes['SEO_ENABLED'] == 'false' ) {
                   return str_replace('&amp;', '&',$this->stock_href_link($page, $parameters, $connection, $add_session_id));
                }
                                ...
                                case 'cPath':
                                        switch(true){
                                                case ($page == FILENAME_DEFAULT):
                                                        $url = $this->make_url($page, 'adesivo-'.$this->get_category_name($p2[1]), $p2[0], $p2[1], '.html');
                                                        break;
 ...
 */        
        function get_information_name($iID){
                switch(true){
                        ...
                        default:
                                if ($this->attributes['USE_SEO_PERFORMANCE_CHECK'] == 'true') $this->performance['NUMBER_QUERIES']++;
                                $sql = "SELECT text_titulo as iName 
                                               FROM ".TABLE_EDIT_TEXT." 
                                               WHERE text_id='".(int)$iID."'  
                                               LIMIT 1";
                                $result = $this->DB->FetchArray( $this->DB->Query( $sql ) );
 ...
 */        
        function strip($string){
			//
                //if ( is_array($this->attributes['SEO_CHAR_CONVERT_SET']) ) $string = strtr($string, array('Ъ'=>'U','в'=>'a'));
          		$string = strtr($string, array('Б'=>'A','А'=>'A','В'=>'A','Г'=>'A','Д'=>'A','Й'=>'E','И'=>'E','К'=>'E','Л'=>'E','Н'=>'I','М'=>'I','О'=>'I','П'=>'I','У'=>'O','Т'=>'O','Ф'=>'O','Х'=>'O','Ц'=>'O','Ъ'=>'U','Щ'=>'U','Ы'=>'U','Ь'=>'U','З'=>'C','б'=>'a','а'=>'a','в'=>'a','г'=>'a','д'=>'a','й'=>'e','и'=>'e','к'=>'e','л'=>'e','н'=>'i','м'=>'i','о'=>'i','п'=>'i','у'=>'o','т'=>'o','ф'=>'o','х'=>'o','ц'=>'o','ъ'=>'u','щ'=>'u','ы'=>'u','ь'=>'u','з'=>'c'));
