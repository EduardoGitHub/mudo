    ...
    switch ($action) {
      ...
      case 'deleteconfirm':
        ...
        if (isset($HTTP_POST_VARS['delete_image']) && ($HTTP_POST_VARS['delete_image'] == 'on')) {
          ...
          if (is_file(DIR_FS_CATALOG_BANNERS . $banner['banners_image'])) {
            if (is_writeable(DIR_FS_CATALOG_BANNERS . $banner['banners_image'])) {
              unlink(DIR_FS_CATALOG_BANNERS . $banner['banners_image']);
            } else {
      ...
      <tr><?php echo tep_draw_form('new_banner', FILENAME_BANNER_MANAGER, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo tep_draw_hidden_field('banners_id', $bID); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_file_field('banners_image') . ' ' . TEXT_BANNERS_IMAGE_LOCAL . '<br>' . DIR_FS_CATALOG_BANNERS . tep_draw_input_field('banners_image_local', (isset($bInfo->banners_image) ? $bInfo->banners_image : '')); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          <tr>
            ...
            <td class="main"><?php echo DIR_FS_CATALOG_BANNERS . tep_draw_input_field('banners_image_target'); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
...
<?php
    $banners_query_raw = "select banners_id, banners_title, banners_image, banners_group, status, expires_date, expires_impressions, date_status_change, date_scheduled, date_added from " . TABLE_BANNERS . " order by banners_group, banners_title";
    $banners_split = new splitPageResults($HTTP_GET_VARS['page'], 100, $banners_query_raw, $banners_query_numrows);
...
?>
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $banners_split->display_count($banners_query_numrows, 100, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_BANNERS); ?></td>
                    <td class="smallText" align="right"><?php echo $banners_split->display_links($banners_query_numrows, 100, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page']); ?></td>
                  </tr>
                  <tr>
    ...
    default:
      if (is_object($bInfo)) {
        ...
        if ( (function_exists('imagecreate')) && ($dir_ok) && ($banner_extension) ) {
          ...
          $contents[] = array('align' => 'center', 'text' => '<br>' . renderChart("includes/charts/includes/FCF_MSColumn3D.swf", "includes/charts/xml/banner_infobox-".$banner_id.".xml", "", "chart1", 300, 300));
        } else {
